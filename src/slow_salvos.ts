...
type CAObject = ({x: number, y: number, w: number, h: number, p: MAPPattern} & ({type: 'sl' | 'other', code: string} | {type: 'glider', dir: 'nw' | 'ne' | 'sw' | 'se', t: number}));
...
function salvoToString(s: Salvo, data: false | CAObject[]): string {
    ...
    let gliders: (CAObject & {type: 'glider'})[] = [];
    for (let obj of data) {
        if (typeof obj !== 'string' && obj.type === 'glider') {
            gliders.push(obj);
            continue;
        }
