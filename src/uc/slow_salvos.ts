...
function _check(target: string, timing: number): undefined | false | string[] {
    let info = c.SALVO_INFO['Slow salvo'];
    let lane = 0;
    let data = getSalvoCollision(info, target, lane, timing);
    if (data === 'no') {
        return;
    ...
    for (; lane < info.laneLimit; lane++) {
        ...
        if (data.some(x => x.type === 'ship' || x.type === 'other')) {
            continue;
        }
        for (let obj of data) {
            if (obj.type === 'sl' || obj.type === 'osc') {
                out.add(obj.code);
                let index = obj.code.indexOf('_');
                let p = base.loadApgcode(obj.code.slice(index + 1));
                for (let i = 0; i < 2; i++) {
                    for (let j = 0; j < 4; j++) {
                        p.rotateRight();
                        out.add(obj.code.slice(0, index + 1) + p.toApgcode());
                    }
                    p.flipHorizontal();
                }
            }
...
function check(target: string): undefined | false | string[] {
    if (target.startsWith('xs')) {
        return _check(target, 0);
    }
    let period = parseInt(target.slice(2));
    let out: string[] = [];
    for (let i = 0; i < period; i++) {
        let value = _check(target, i);
        if (!value) {
            return value;
        }
        out.push(...value);
...
let cache: {[key: string]: false | string[]} = {};

let rles: string[] = [];
import * as fs from 'node:fs/promises';
for (let file of await fs.readdir('.')) {
    if (file.endsWith(`_bits_${process.argv[2]}.txt`)) {
        rles.push(...(await fs.readFile(file)).toString().split('\n').filter(x => x));
    }
}

console.log(`Checking ${rles.length} objects`);
let start = performance.now();
let prevUpdateTime = start;
let prevChecked = 0;

for (let i = 0; i < rles.length; i++) {
    let now = performance.now();
    if ((now - prevUpdateTime) / 1000 > 3) {
        console.log(`${i}/${rles.length} (${(i / rles.length * 100).toFixed(3)}%) complete (${((i - prevChecked)/((now - prevChecked) / 1000)).toFixed(3)} objects/second current, ${(i / ((now - start) / 1000)).toFixed(3)} overall)`);
        prevUpdateTime = now;
        prevChecked = i;
    }
    let p = base.loadRLE(rles[i]);
    let code = `xs${p.population}_${p.toCanonicalApgcode()}`;
    if (code in cache) {
        if (cache[code] === false) {
            continue;
        } else {
            let found = false;
            for (let obj of cache[code]) {
                if (obj in cache) {
                    if (cache[obj] === false) {
                        found = true;
                        break;
                    ...
                    let data = check(obj);
                    if (data === undefined) {
                        console.log(`\x1b[91mSkipping: ${obj}\x1b[0m`);
                        continue;
                    }
                    cache[obj] = data;
                    if (data === false) {
            ...
            if (!found) {
                console.log(`\x1b[92mSolution found: ${code}\x1b[0m`);
            }
        }
    }
    let data = check(code);
    if (data === undefined) {
        console.log(`\x1b[91mSkipping: ${code}\x1b[0m`);
        continue;
    }
    cache[code] = data;
