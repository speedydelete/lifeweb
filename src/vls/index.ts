...
export async function main() {
    ...
    try {
        let command = `gcc --std=c2x -Wall -Wextra -Werror -Wpedantic -Wno-unused-function -Wno-unknown-pragmas ${options['profile'] ? '-pg -O3' : (options['g'] || options['gdb'] ? '-g -O0' : '-O3')} -march=native -mtune=native -flto -fno-stack-protector -fomit-frame-pointer -o '${execPath}' '${getPath('src/vls/index.c')}'`;
