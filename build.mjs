...
async function buildEditor() {
    buildMain();
    let data = (await fs.readFile(path('src/editor.html'))).toString();
    let match = data.match(/var BUILD_NUMBER=(\d+);/);
    if (!match) {
        console.error(`Error: No build number in src/editor.html!`);
        throw new Error(`No build number!`);
    }
    data = data.replace(match[0], `var BUILD_NUMBER=${parseInt(match[1]) + 1};`);
    await fs.writeFile(path('src/editor.html'), data);
    ...
    await esbuild.build({
        entryPoints: [path('src/editor.ts')],
        bundle: true,
